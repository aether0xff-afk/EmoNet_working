...
이 프로젝트의 핵심 의도는 ``LLM이 더 그럴듯한 답변을 하게 만들자''가 아니다. 사용자가 만들고자 한 것은 입력 문장을 보고 바로 ``분노'', ``슬픔'', ``불안'' 같은 이름을 붙이는 모델도 아니다. 더 중요한 관심은 입력이 들어왔을 때 모델 내부에서 무엇이 실제로 움직였는지를 보는 것이다.

본 논문에서 말하는 trace는 어렵게 말하면 신경 활성 추적이지만, 쉽게 말하면 입력이 모델 안을 지나가며 남긴 내부 발자국이다. 어떤 문장을 읽었을 때 어떤 뉴런들이 켜졌는지, 그 켜짐이 얼마나 오래 이어졌는지, 어떤 경로가 강하게 남았는지를 시간 순서대로 기록한 것이다. 즉 trace는 최종 답변이나 감정 라벨이 아니라, 답변이 나오기 전 내부에서 생긴 감정 흐름의 기록이다.

예를 들어 같은 ``화가 난다''라는 상황이라도 누군가는 바로 따지고 싶고, 누군가는 참고 넘기려 하며, 누군가는 관계가 망가질까 봐 불안해한다. 겉으로 붙는 라벨은 모두 분노일 수 있지만, 내부 흐름은 다를 수 있다. EmoNet v3.1은 바로 이 차이를 trace 안에서 찾고자 한다.

따라서 v3.1에서 중요한 것은 다음 질문들이다.

\begin{enumerate}
  \item 입력이 들어왔을 때 모델 내부에 감정 흐름이 실제로 생기는가?
  \item 그 흐름이 너무 빨리 사라지거나, 반대로 너무 많은 뉴런을 한꺼번에 켜지는 않는가?
  \item 그 흐름 안에 긍정/부정, 통제감, 상대와의 관계, 행동 충동 같은 차이가 남는가?
  \item trace의 특정 부분을 바꾸면 최종 응답의 감정 해석도 함께 바뀌는가?
  \item trace에서 특정 감정 단서를 약하게 만들면 원래 감정 신호도 약해지는가?
\end{enumerate}

이 질문들이 모두 어느 정도 만족되어야만 ``trace가 단순한 기록이 아니라 감정 상태를 담은 표현일 수 있다''고 말할 수 있다.
