...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--record-ids", required=True, help="Comma-separated ids")
...
def main() -> None:
    ...
    requested_ids = [item.strip() for item in str(args.record_ids).split(",") if item.strip()]
    ...
    selected["_requested_order"] = selected[args.record_id_column].astype(str).map({rid: idx for idx, rid in enumerate(requested_ids)})
    selected = selected.sort_values("_requested_order").drop(columns=["_requested_order"])
