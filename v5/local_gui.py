...
STARTER_MESSAGE = os.environ.get(
    "EMONET_RUCA_STARTER",
    "너 왔네. ...조금 늦게 봤어.\n[ACTION] 시선을 피했다가 다시 올려다본다.\n무슨 말부터 해야 할지 모르겠는데, 그래도 네가 먼저 말 안 하면 내가 더 신경 쓰일 것 같아서.",
)
...
def _starter_messages() -> list[dict[str, Any]]:
    return [{"role": "assistant", "content": STARTER_MESSAGE, "source": "starter"}]


def _ensure_starter_message() -> None:
    if not _messages:
        _messages.extend(_starter_messages())
...
def _estimate_cost(input_tokens: int, output_tokens: int) -> float:
    return (input_tokens * LLM_INPUT_PRICE + output_tokens * LLM_OUTPUT_PRICE) / 1_000_000.0
...
def _generate_ai_user_message(*, api_key: str | None, scenario: str, turn_index: int) -> tuple[str, dict[str, Any]]:
...
def _state_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "api": {
            ...
            "base_url": LLM_BASE_URL,
            "model": LLM_MODEL,
            "input_price_per_mtok": LLM_INPUT_PRICE,
            "output_price_per_mtok": LLM_OUTPUT_PRICE,
        },
...
<body>
  <div class="app">
    <aside>
      ...
      <input id="apiKey" type="password" autocomplete="off" placeholder="usually empty for local Ollama" />
