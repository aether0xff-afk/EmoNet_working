...
CONDITION_SPECS: dict[str, dict[str, object]] = {
    ...
    "episode_trace": {
        "group": "episode",
        "description": "LLM-interpreted emotion episode only.",
        "needs_profile": True,
        "needs_episode": True,
    },
    "episode_trace_v3": {
...
def build_condition_prompt(condition: str, input_text: str, profile: dict[str, object] | None) -> tuple[str, str]:
    ...
    if condition == "hybrid_episode":
        return build_conditioned_generation_prompt(
            input_text=input_text,
            profile=profile,
            conditioning_mode="hybrid_episode",
            template_path=None,
        )
...
def main() -> None:
    ...
    parser.add_argument("--episode-record-id-column", default="sample_id")
    ...
    needs_episode = any(bool(CONDITION_SPECS[name].get("needs_episode")) for name in conditions)
    if needs_episode and episode_dir is None:
        raise ValueError("--episode-dir is required when using episode conditions")
    ...
    for idx, record in enumerate(input_df.to_dict(orient="records"), start=1):
        ...
        if any(bool(CONDITION_SPECS[name]["needs_profile"]) for name in conditions):
            try:
                ...
                if needs_episode and profile is not None:
                    episode_path = resolve_episode_payload_path(
                        episode_dir=episode_dir,
                        record=record,
                        explicit_record_id_column=args.episode_record_id_column,
                    )
                    profile = augment_profile_with_episode(
                        profile,
                        load_episode_payload(episode_path),
