...
class EmoNetSmokeTests(unittest.TestCase):
    ...
    def test_command_probe_branch_reports_stats(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            stdout = io.StringIO()
            with patch("emonet.legacy_cli.build_model", return_value=ProbeModel()), contextlib.redirect_stdout(stdout):
    ...
    def test_label_subset_with_local_model(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            with patch("emonet.legacy_cli.ensure_model_server_ready"), patch(
                "emonet.legacy_cli.call_openai_compatible_chat", side_effect=[generation_response, *block_responses]
            ):
                label_subset_with_local_model(
                    df=df,
                    output_csv=output_csv,
    ...
    def test_label_subset_with_local_model_resume(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            with patch("emonet.legacy_cli.ensure_model_server_ready"), patch(
                "emonet.legacy_cli.call_openai_compatible_chat", side_effect=row1_responses
            ):
                label_subset_with_local_model(
                    df=df,
                    output_csv=output_csv,
            ...
            with patch("emonet.legacy_cli.ensure_model_server_ready"), patch(
                "emonet.legacy_cli.call_openai_compatible_chat", side_effect=row2_responses
            ):
                label_subset_with_local_model(
                    df=df,
                    output_csv=output_csv,
    ...
    def test_request_json_response_retries_on_schema_validation(self) -> None:
        with patch(
            "emonet.llm_api.call_chat_with_usage",
            side_effect=[
                ("{\"s\": {\"dim0\": 0.2}}", {}),
    ...
    def test_request_json_response_retries_once(self) -> None:
        with patch("emonet.llm_api.call_chat_with_usage", side_effect=[("not json", {}), ("{\"ok\": true}", {})]):
            payload, raw = request_json_response(
                base_url="http://127.0.0.1:8000/v1",
                model_name="gpt-oss-20b",
                prompt="test",
    ...
    def test_command_generate_response(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            with patch("emonet.legacy_cli.ensure_model_server_ready"), patch(
                "emonet.legacy_cli.build_model", return_value=self.FakeGenerativeModel()
            ), patch("emonet.legacy_cli.LinearZtoSDecoder.load", return_value=self.FakeDecoder()), patch(
                "emonet.legacy_cli.call_openai_compatible_chat", return_value="조금 쉬어가면서 우선순위를 다시 정리해 보세요."
            ):
    ...
    def test_command_generate_response_retries_invalid_plain_text(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            with patch("emonet.legacy_cli.ensure_model_server_ready"), patch(
                ...
                "emonet.legacy_cli.call_openai_compatible_chat",
                side_effect=[
    ...
    def test_command_generate_response_accepts_grad_tensor_z(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            with patch("emonet.legacy_cli.ensure_model_server_ready"), patch(
                "emonet.legacy_cli.build_model", return_value=GradTensorModel()
            ), patch("emonet.legacy_cli.LinearZtoSDecoder.load", return_value=self.FakeDecoder()), patch(
                "emonet.legacy_cli.call_openai_compatible_chat", return_value="조금만 정리하고 쉬자."
            ):
                command_generate_response(args)
    ...
    def test_command_generate_response_batch(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            with patch("emonet.legacy_cli.ensure_model_server_ready"), patch(
                ...
                "emonet.legacy_cli.call_openai_compatible_chat", side_effect=["응답 하나", "응답 둘"]
            ):
    ...
    def test_command_e2e_check_success(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            with patch("emonet.legacy_cli.ensure_model_server_ready"), patch(
                ...
                "emonet.legacy_cli.call_openai_compatible_chat", return_value="조금 쉬면서 호흡을 가다듬어 보세요."
            ):
    ...
    def test_command_e2e_check_records_llm_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            with patch("emonet.legacy_cli.build_model", return_value=self.FakeGenerativeModel()), patch(
                "emonet.legacy_cli.LinearZtoSDecoder.load", return_value=self.FakeDecoder()
            ), patch(
                "emonet.legacy_cli.ensure_model_server_ready",
