...
def analyze_turn_context(
    *,
    ...
    active_event_type = _active_event_type(event_type, scheduled_event_type)

    if active_event_type in {"no_reply", "silence_tick", "long_silence"}:
        context_event_type = active_event_type
        ...
        context_event_type = "implementation_request"
...
def _active_event_type(event_type: str, scheduled_event_type: str) -> str:
    explicit = str(event_type or "").strip()
    scheduled = str(scheduled_event_type or "").strip()
    if explicit and explicit != "user_message":
        return explicit
    return scheduled or explicit or "user_message"
