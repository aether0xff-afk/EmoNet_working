...
def generate_llm_response(prompt: str, config: LLMConfig) -> LLMResponse:
    ...
    if provider == "anthropic":
        ...
        text, usage = _call_gemini_generate_content(prompt=prompt, config=config, api_key=api_key)
...
def _call_gemini_generate_content(prompt: str, config: LLMConfig, api_key: str | None) -> tuple[str, dict[str, int]]:
    if not api_key:
        raise ValueError("Gemini provider requires an API key")
    url = config.base_url.rstrip("/") + f"/models/{config.model_name}:generateContent"
    payload: dict[str, Any] = {
        "systemInstruction": {
            "parts": [
                {
    ...
    return text, _normalize_gemini_usage(response_payload.get("usageMetadata") or {})
...
def _normalize_gemini_usage(usage: dict[str, Any]) -> dict[str, int]:
    return {
