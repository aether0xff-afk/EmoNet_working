...
from .emonet_adapter import EmoNetTraceResult, create_emonet_runtime, infer_emonet_trace
...
from .trace_episode import build_episode_prompt, build_response_prompt_from_episode
...
class RucaPipeline:
    def __init__(
        ...
        *,
        ...
        response_timing_mode: str = "event",
    ) -> None:
        ...
        self.response_timing_mode = str(response_timing_mode or "event").strip().lower()
        self._emonet_runtime = create_emonet_runtime() if self.use_emonet else None
    ...
    def _run_event(
        ...
        *,
        ...
        emotion_event_type = "no_reply" if event.event_type in {"typing", "answering", "processing", "no_reply", "silence_tick", "long_silence"} else event.event_type
        ...
        if self.use_emonet:
            try:
                emonet_trace = infer_emonet_trace(
                    _text_for_emonet(event, reference_text),
                    event_type=event.event_type,
                    elapsed_minutes=event.elapsed_minutes,
                    runtime=self._emonet_runtime,
                )
        ...
        response_decision = _apply_neural_response_timing(
            mode=self.response_timing_mode,
            event=event,
        ...
        if response_decision.action == "send_message":
            ...
            if self.use_llm:
                try:
                    llm_prompt = response_prompt
                    if emonet_trace is not None:
                        episode_prompt = build_episode_prompt(
                            user_text=reference_text,
                            event_type=event.event_type,
                            elapsed_minutes=event.elapsed_minutes,
                            trace_record=emonet_trace.to_record(),
                        )
                        episode_response = generate_llm_response(episode_prompt, self.llm_config or LLMConfig())
                        trace_episode = episode_response.text
                        llm_prompt = build_response_prompt_from_episode(
                            user_text=reference_text,
                            event_type=event.event_type,
                            elapsed_minutes=event.elapsed_minutes,
                            episode_text=trace_episode,
                        )
                        response_prompt = llm_prompt
                    llm_response = generate_llm_response(llm_prompt, self.llm_config or LLMConfig())
                    assistant_text = llm_response.text
                    expression_mode = "trace_episode_llm" if trace_episode is not None else "llm"
                except Exception as exc:
                    llm_error = str(exc)
        ...
        debug_record = {
            ...
            "response_timing_mode": self.response_timing_mode,
...
def run_turn(
    ...
    *,
    ...
    pipeline = RucaPipeline(
        ...
        response_timing_mode=response_timing_mode,
    )
...
def _text_for_emonet(event: RucaEvent, reference_text: str) -> str:
    ...
    return f"[{event.event_type} after {event.elapsed_minutes:.1f} minutes without new user speech] {reference_text}".strip()


def _apply_neural_response_timing(
    *,
    mode: str,
    event: RucaEvent,
    ...
    if mode != "neural" or emonet_trace is None:
        return decision
    if event.event_type != "user_message" or decision.action != "send_message":
        return decision

    record = emonet_trace.to_record()
    profile = dict(record.get("trace_profile") or {})
    memory = dict(record.get("neuron_memory") or {})
    neuron_count = max(float(profile.get("neuron_count") or 1.0), 1.0)
    mean_active = float(profile.get("mean_active_nodes") or 0.0)
    activity_ratio = mean_active / neuron_count
    stored_memory = int(memory.get("stored_memory_count") or 0)
    tick_index = int(profile.get("tick_index") or 0)

    should_speak = activity_ratio >= 0.44 or stored_memory > 0 or tick_index >= 4
    if should_speak:
        return ResponseDecision(
            action="send_message",
            intensity=max(decision.intensity, min(1.0, activity_ratio)),
            reason=f"신경망 활동 비율 {activity_ratio:.3f}가 충분해 말이 밖으로 나오는 타이밍으로 판단했다.",
        )
    return ResponseDecision(
        action="update_internal_only",
        intensity=min(1.0, activity_ratio),
        reason=f"신경망 활동 비율 {activity_ratio:.3f}가 낮아 입력을 내부 상태로만 흡수한다.",
    )
