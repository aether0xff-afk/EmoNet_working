...
from ruca_engine.llm_client import LLMConfig, LLMResponse, generate_llm_response
...
class RucaEngineTests(unittest.TestCase):
    ...
    def test_gemini_provider_calls_generate_content(self) -> None:
        captured = {}

        def fake_post_json(*, url, payload, headers, timeout_sec):
            captured["url"] = url
        ...
        with patch("ruca_engine.llm_client._post_json", side_effect=fake_post_json):
        ...
        self.assertEqual(response.text, "응, Gemini로 연결됐어.")
        self.assertEqual(captured["url"], "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent")
    ...
    def test_cli_interactive_keeps_emonet_ticks_alive(self) -> None:
        completed = subprocess.run(
            [
                sys.executable,
                "-m",
        ...
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("event_kind=user_message tick=0 stim_dim=8", completed.stdout)
        self.assertIn("event_kind=typing tick=1 stim_dim=8", completed.stdout)
